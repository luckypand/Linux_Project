...
void HttpConn::init(int fd, const sockaddr_in& addr) {
    ...
    addr_ = addr;
    fd_ = fd;
    writeBuff_.RetrieveAllToStr();
    readBuff_.RetrieveAllToStr();
    isClose_ = false;
...
ssize_t HttpConn::write(int* saveErrno) {
    ...
    do {
        ...
        if(iov_[0].iov_len + iov_[1].iov_len  == 0) { break; } /* 传输结束 */
            ...
            iov_[0].iov_len -= len; 
            std::string remain = writeBuff_.RetrieveAllToStr();
            if(static_cast<size_t>(len) < remain.size()) {
                writeBuff_.Append(remain.data() + len, remain.size() - len);
            }
        }
