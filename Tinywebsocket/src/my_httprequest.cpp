...
bool HttpRequest::parse(Buffer& buff) {
    ...
    while(buff.ReadableBytes() && state_ != FINISH) {
        const char* begin = buff.BeginRead();
        const char* end = buff.BeginWrite();
        // 从buff中的读指针开始到读指针结束，这块区域是未读取得数据并去处"\r\n"，返回有效数据得行末指针
        const char* lineEnd = search(begin, end, CRLF, CRLF + 2);
        // 转化为string类型
        ...
        */
        ...
        if(lineEnd == end) { break; } // 读完了
        size_t consumeLen = static_cast<size_t>((lineEnd + 2) - begin);
        std::string remain = buff.RetrieveAllToStr();
        if(consumeLen < remain.size()) {
            buff.Append(remain.data() + consumeLen, remain.size() - consumeLen);
        }
    }
