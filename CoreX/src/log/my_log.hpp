...
private:
    ...
    int ScanInitSeq_(const std::string& modulePath); // 扫描目录获取当天已有文件最大序号
    ...
    std::unordered_map<std::string, int> moduleFileSeq_;        // 模块名→当前文件序号（跨运行递增）
